...
This code was written by Seungwon Do (dodoseung) and modified by Jinhyung park, Gyucheol Shim, Hyunwoo Lee
"""
...
def model(state_true, state_cog, para_bump, para_icp, para_env, sigma):
    ...
    threshold_id, c_mu, c_sigma, _nu, _delta, _p = para_icp
    ...
    target_vel_x, target_vel_y = visual.visual_speed_noise(target_vel_x, target_vel_y, sigma)

    # Target information at RP
    target_pos_x, target_vel_x = motor.boundary(Tp, target_pos_x, target_vel_x, interval, boundary_width, target_radius)
    target_pos_y, target_vel_y = motor.boundary(Tp, target_pos_y, target_vel_y, interval, boundary_height, target_radius)
    targetInfo = [target_pos_x, target_pos_y, target_vel_x, target_vel_y]
    ...
    vel_x = otg_ideal_x[1, :pred_horizon + 1].copy()
    vel_y = otg_ideal_y[1, :pred_horizon + 1].copy()

    # Set the mouse noise
    mouse_gain = mouse.gain_func_can((vel_x ** 2 + vel_y ** 2) ** (1 / 2))
    for idx in range(pred_horizon + 1):
        if mouse_gain[idx] == 0: mouse_gain[idx] = np.finfo(float).tiny

    # Get the acceleration
    acc_x = ((vel_x / mouse_gain)[1:] - (vel_x / mouse_gain)[:-1]) / interval
    acc_y = ((vel_y / mouse_gain)[1:] - (vel_y / mouse_gain)[:-1]) / interval

    # Get the ideal hand pos
    _, _, h_pos_x_ideal, h_pos_y_ideal = limb.mouse_noise(vel_x / mouse_gain, vel_y / mouse_gain, h_pos_x, h_pos_y, forearm, mouse_gain, interval)
    h_pos_dx = h_pos_x_ideal - h_pos_x
    h_pos_dy = h_pos_y_ideal - h_pos_y

    # Set the motor noise
    _, _, vel_x, vel_y = limb.motor_noise(vel_x / mouse_gain, vel_y / mouse_gain, pred_horizon, 0, 0, nc, interval)

    # Set the mouse noise
    pos_dx_mouse, pos_dy_mouse, h_pos_x, h_pos_y = limb.mouse_noise(vel_x.copy(), vel_y.copy(), h_pos_x, h_pos_y, forearm, mouse_gain, interval)
    pos_dx = ((vel_x * mouse_gain)[1:] + (vel_x * mouse_gain)[:-1]) / 2 * interval
    pos_dy = ((vel_y * mouse_gain)[1:] + (vel_y * mouse_gain)[:-1]) / 2 * interval
    vel_mouse_noise_x = (sum(pos_dx_mouse) - sum(pos_dx)) / (pred_horizon * interval)
    vel_mouse_noise_y = (sum(pos_dy_mouse) - sum(pos_dy)) / (pred_horizon * interval)
    pos_dx, pos_dy, vel_x, vel_y = limb.motor_noise(vel_x * mouse_gain, vel_y * mouse_gain, pred_horizon, vel_mouse_noise_x, vel_mouse_noise_y, [0, 0], interval)

    # Ideal last executed cursor position and velocity for next BUMP
    cursor_dx_dy = [otg_ideal_x[0, pred_horizon] - otg_ideal_x[0, 0], otg_ideal_y[0, pred_horizon] - otg_ideal_y[0, 0],
                    otg_ideal_x[1, pred_horizon] - otg_ideal_x[1, 0], otg_ideal_y[1, pred_horizon] - otg_ideal_y[1, 0]]

    # Ideal cursor velocity
    vel_p = [otg_ideal_x[1, pred_horizon], otg_ideal_y[1, pred_horizon]]
    ...
    if threshold_id:
        ...
        cursor_vel = np.array([np.sum(pos_dx) / tc, np.sum(pos_dy) / tc])
    ...
    for idx in range(len(pos_dx)):
        if c_true_pos_x + sum(pos_dx[:idx + 1]) <= 0:
            ...
            vel_x[idx + 1] = 0
        elif c_true_pos_x + sum(pos_dx[:idx + 1]) >= boundary_width:
            pos_dx[idx] = boundary_width - (c_true_pos_x + sum(pos_dx[:idx]))
            vel_x[idx + 1] = 0

        if c_true_pos_y + sum(pos_dy[:idx + 1]) <= 0:
            pos_dy[idx] = -(c_true_pos_y + sum(pos_dy[:idx]))
            vel_y[idx + 1] = 0
        elif c_true_pos_y + sum(pos_dy[:idx + 1]) >= boundary_height:
            pos_dy[idx] = boundary_height - (c_true_pos_y + sum(pos_dy[:idx]))
            vel_y[idx + 1] = 0

    return pos_dx, pos_dy, vel_x, vel_y, time_total, np.array(cursor_dx_dy), acc_sum, h_pos_x, h_pos_y, vel_p,\
           targetInfo, np.array([h_pos_dx, h_pos_dy])
