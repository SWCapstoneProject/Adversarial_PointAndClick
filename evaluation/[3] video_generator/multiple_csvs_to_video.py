...
Video Generating & Comparing Module
 - Non-Adversarial Environment Trained Agent (Baseline) VS Adversarial-Environment Trained Agent

1) Compares the result of an agent that was trained in an adversarial environment
   with a baseline agent that was trained in a non-adversarial environment
...
3) Each input csv file must contain per timestep cursor position, target position, and target radius data for each trial
...
CSV_OURS = 'trajectory_data/trajectory_adv_my_agent_4090000.csv'
CSV_BASELINE = 'trajectory_data/trajectory_non_adv_agent_4310000.csv'
    ...
    for user_number in range(NUM_USERS):
        for task_number in range(NUM_TASKS):
            for trial_number in tqdm(range(1, NUM_TRIALS + 1)):
                ...
                # 1. Get trajectory_data frames of my agent and store it to local_frames_ours buffer
                ...
                # 2. Get trajectory_data frames of baseline agent and store it to local_frames_baseline buffer
                ...
                # 3. Merge the frames from each agent
                ours_ptr = 0
                baseline_ptr = 0
                ...
                if ours_ptr == len(local_frames_ours):
                    # 3-1 Ours frame exhausted - use the last frame
                    ...
                    # 3-2 Baseline frame exhausted - use the last frame
                ...
                # 4. Add the same last frame for some iterations, to prevent from video finishing immediately
